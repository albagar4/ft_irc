...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/27 11:29:22 by albagar4          #+#    #+#             */
/*   Updated: 2024/11/27 11:29:23 by albagar4         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::parseNick(std::string buffer, Client &client) {
    try {
        ...
        if (buffer.find('#') != buffer.npos || buffer.find(':') != buffer.npos ||
            buffer.find(' ') != buffer.npos || isdigit(buffer[0]))
            throw 432;
        ...
        if (code == 451) err(ERR_NOTREGISTERED, this->getHostname(), client);
        if (code == 431) err(ERR_NONICKNAMEGIVEN, this->getHostname(), client);
        if (code == 432) err(ERR_ERRONEUSNICKNAME, this->getHostname(), client, buffer);
        if (code == 433) err(ERR_NICKNAMEINUSE, this->getHostname(), client, buffer);
    }
}
